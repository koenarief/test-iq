...
export default function Test({ discTest, questions = [] }) {
    ...
    /*
     * Hitung sisa waktu berdasarkan started_at dari server.
     * Durasi DISC/Gaya Kerja = 30 menit = 1800 detik.
     */
    const calculateRemainingSeconds = () => {
        ...
        let dateStr = discTest.started_at;
    ...
    /*
     * Kondisi test sudah terkunci.
     * Setelah timeout/final submit, peserta tidak boleh mengubah jawaban lagi.
     */
    const testLocked =
        timeLeft <= 0 ||
        processing ||
    ...
    /*
     * Handler memilih:
     * P = paling menggambarkan
     * K = paling tidak menggambarkan
     */
    const handleSelectOption = (statementIndex, type) => {
        if (
            testLocked ||
        ...
        if (type === 'P') {
            if (updatedMost === statementIndex) {
                ...
                /*
                 * P dan K tidak boleh berada
                 * pada pernyataan yang sama.
                 */
                if (updatedLeast === statementIndex) {
                    updatedLeast = null;
                }
        ...
        /*
         * Update ref dan state bersamaan.
         *
         * answersRef penting agar bila timeout terjadi
         * tepat setelah participant memilih jawaban,
         * submit tetap mendapatkan jawaban terbaru.
         */
    ...
    /*
     * Apakah soal aktif sudah lengkap.
     */
    const isCurrentQuestionAnswered = Boolean(
        currentAnswer.most_choice !== null &&
        currentAnswer.least_choice !== null
    );

    /*
     * Hitung jumlah soal yang benar-benar lengkap P + K.
     */
    const totalAnswered = Object.values(answers).filter(
        (answer) =>
            answer?.most_choice !== null &&
            answer?.least_choice !== null
    ).length;
    ...
    /*
     * Satu jalur submit untuk:
     *
     * submitted = peserta menyelesaikan secara normal
     * timeout   = timer mencapai 00:00
     */
    const submitTest = (reason = 'submitted') => {
        ...
        /*
         * Submit manual hanya boleh jika seluruh soal lengkap.
         *
         * Timeout boleh mengirim jawaban parsial.
         * Validasi timeout akan kita tangani juga di backend.
         */
        if (reason === 'submitted') {
        ...
        /*
         * Guard diset SEBELUM request.
         * Ini mencegah double click / double timeout request.
         */
        ...
        router.post(
            ...
            },
            {
                ...
                onError: () => {
                    /*
                     * Kalau request gagal, boleh dicoba kembali.
                     *
                     * Untuk timeout, timer tetap 00:00 sehingga
                     * halaman tetap terkunci.
                     */
    ...
    /*
     * Submit manual dari tombol terakhir.
     */
    const handleSubmitTest = () => {
     ...
     * TIMER.
     *
     * Jangan hanya mengurangi state prev - 1.
     * Setiap tick hitung kembali berdasarkan started_at server.
     *
     * Ini membuat timer lebih tahan terhadap:
    ...
    useEffect(() => {
        ...
        /*
         * Cek langsung saat halaman pertama kali dibuka.
         *
         * Jika ternyata waktunya sudah habis sebelum page selesai load,
         * langsung jalankan timeout.
         */
        tick();
    ...
    return (
        <PublicLayout>
            <Head title={`Tes Gaya Kerja - Soal #${currentIndex + 1}`} />
                            ...
                            >
                                ...
                                <div className="space-y-3 mb-8">
                                    {statements.map((statementText, index) => {
                                        ...
                                        const isMost =
                                            currentAnswer.most_choice ===
                                            statementNum;

                                        const isLeast =
                                            currentAnswer.least_choice ===
                                            statementNum;
                                            ...
                                            >
                                                ...
                                                <div className="flex items-center gap-2 self-end sm:self-center shrink-0">
                                                    ...
                                                    <button
                                                        ...
                                                        className={`px-4 py-2 rounded-xl text-xs font-bold font-mono transition-all duration-200 flex items-center gap-1.5 disabled:opacity-40 disabled:cursor-not-allowed ${
                                                            isMost
                                                                ? 'bg-blue-600 text-white shadow-md shadow-blue-600/30 scale-105 border border-blue-400'
                                                                : 'bg-zinc-800/60 text-zinc-400 hover:text-white hover:bg-zinc-800 border border-zinc-700/50'
                                                        }`}
                                ...
                                <div className="p-3.5 rounded-xl bg-zinc-950/60 border border-zinc-800/80 mb-8 flex items-center gap-3 text-xs text-zinc-400">
                                    ...
                                    <span>
                                        Pilih <strong>P</strong> pada
                                        pernyataan yang{' '}
                                        <em>paling menggambarkan</em> Anda,
                                        dan pilih <strong>K</strong> pada
                                        pernyataan yang{' '}
                                        <em>paling tidak menggambarkan</em>{' '}
                                        Anda.
                                    </span>
                                </div>
