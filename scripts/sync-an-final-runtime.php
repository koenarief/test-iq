...
$expectedDatabase = trim((string) getenv('IST_AN_RUNTIME_DATABASE'));
$apply = getenv('IST_AN_RUNTIME_APPLY') === '1';
$connection = (string) config('database.default');
$configuredDatabase = (string) config("database.connections.{$connection}.database");

if ($expectedDatabase === '') {
    throw new RuntimeException('IST_AN_RUNTIME_DATABASE wajib diisi dengan nama database target yang eksplisit.');
}

if (! app()->environment('local')
    || $connection !== 'mysql'
    || ! hash_equals($expectedDatabase, $configuredDatabase)) {
    throw new RuntimeException('Environment, connection, atau configured database tidak memenuhi guard runtime AN.');
}
...
if ($activeDatabase === '' || ! hash_equals($expectedDatabase, $activeDatabase)) {
    throw new RuntimeException('Database aktif tidak cocok dengan IST_AN_RUNTIME_DATABASE; sinkronisasi dihentikan.');
}

$dataset = json_decode(
    (string) file_get_contents(database_path('data/ist-final-staging/an.json')),
    true,
    512,
    JSON_THROW_ON_ERROR,
);

if (($dataset['subtest_code'] ?? null) !== 'AN'
    ...
    throw new RuntimeException('Canonical staging AN tidak valid atau belum human-reviewed dan inactive.');
}
...
if ($canonicalQuestions->count() !== 13
    ...
    throw new RuntimeException('Canonical staging AN harus memuat satu example dan dua belas scored questions.');
}
...
foreach ($canonicalQuestions as $canonical) {
    ...
    if (($canonical['answer_type'] ?? null) !== 'single_choice'
        ...
        throw new RuntimeException('Canonical staging AN mempunyai kontrak opsi atau answer type yang tidak valid.');
    }
    ...
    foreach ($options as $option) {
        ...
        if (($option['score'] ?? null) !== $expectedScore) {
            throw new RuntimeException('Canonical staging AN mempunyai score binary yang tidak konsisten.');
        }
    }
    ...
    if (($canonical['kind'] ?? null) === 'scored') {
        ...
        if (! is_string($difficulty) || ! array_key_exists($difficulty, $difficultyCounts)) {
            throw new RuntimeException('Canonical staging AN mempunyai difficulty scored yang tidak valid.');
        }
...
if ($difficultyCounts !== ['easy' => 4, 'medium' => 5, 'hard' => 3]
    || $weightedMaximum !== 23) {
    throw new RuntimeException('Difficulty atau weighted maximum canonical AN tidak sesuai kontrak 4/5/3 dan 23.');
}
...
$synchronize = static function (bool $write) use ($canonicalQuestions, $dataset): array {
    ...
    if (! $subtest || (int) $subtest->question_count !== 12 || (int) $subtest->duration_seconds !== 240) {
        throw new RuntimeException('Master subtest AN aktif tidak sesuai kontrak 12 soal dan 240 detik.');
    }
    ...
    if ($questions->count() !== 13) {
        throw new RuntimeException('Master AN aktif harus memuat tepat tiga belas record.');
    }
    ...
    foreach ($questions as $question) {
        ...
        if (! is_array($canonical)
            ...
            throw new RuntimeException("Identitas master AN {$key} berbeda dari canonical.");
        }
        ...
        if ($options->count() !== 5) {
            throw new RuntimeException("Master AN {$key} tidak mempunyai tepat lima opsi aktif.");
        }
        ...
        foreach ($options as $option) {
            ...
            if (! is_array($canonicalOption)) {
                throw new RuntimeException("Opsi AN {$key}/{$option->option_key} tidak tersedia pada canonical.");
            }
...
if ($apply) {
    ...
    if (array_sum($postflight) !== 0) {
        throw new RuntimeException('Postflight AN masih menemukan drift setelah sinkronisasi.');
    }
} else {
