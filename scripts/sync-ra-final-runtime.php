...
$expectedDatabase = trim((string) getenv('IST_RA_RUNTIME_DATABASE'));
$apply = getenv('IST_RA_RUNTIME_APPLY') === '1';
$connection = (string) config('database.default');
$configuredDatabase = (string) config("database.connections.{$connection}.database");

if ($expectedDatabase === '') {
    throw new RuntimeException('IST_RA_RUNTIME_DATABASE wajib diisi dengan nama database target yang eksplisit.');
}

if (! app()->environment('local')
    || $connection !== 'mysql'
    || ! hash_equals($expectedDatabase, $configuredDatabase)) {
    throw new RuntimeException('Environment, connection, atau configured database tidak memenuhi guard runtime RA.');
}
...
if ($activeDatabase === '' || ! hash_equals($expectedDatabase, $activeDatabase)) {
    throw new RuntimeException('Database aktif tidak cocok dengan IST_RA_RUNTIME_DATABASE; sinkronisasi dihentikan.');
}

$dataset = json_decode(
    (string) file_get_contents(database_path('data/ist-final-staging/ra.json')),
    true,
    512,
    JSON_THROW_ON_ERROR,
);

if (($dataset['subtest_code'] ?? null) !== 'RA'
    ...
    throw new RuntimeException('Canonical staging RA tidak valid atau belum human-reviewed dan inactive.');
}
...
if ($canonicalQuestions->count() !== 13
    ...
    throw new RuntimeException('Canonical staging RA harus memuat satu example dan dua belas scored questions.');
}
...
foreach ($canonicalQuestions as $canonical) {
    ...
    if (($canonical['answer_type'] ?? null) !== 'numeric'
        ...
        throw new RuntimeException('Canonical staging RA mempunyai answer type, opsi, atau numeric key yang tidak valid.');
    }
    ...
    if (($canonical['kind'] ?? null) === 'scored') {
        ...
        if (! is_string($difficulty) || ! array_key_exists($difficulty, $difficultyCounts)) {
            throw new RuntimeException('Canonical staging RA mempunyai difficulty scored yang tidak valid.');
        }
...
if ($difficultyCounts !== ['easy' => 4, 'medium' => 5, 'hard' => 3]
    || $weightedMaximum !== 23) {
    throw new RuntimeException('Difficulty atau weighted maximum canonical RA tidak sesuai kontrak 4/5/3 dan 23.');
}
...
$synchronize = static function (bool $write) use ($canonicalQuestions, $dataset): array {
    ...
    if (! $subtest || (int) $subtest->question_count !== 12 || (int) $subtest->duration_seconds !== 360) {
        throw new RuntimeException('Master subtest RA aktif tidak sesuai kontrak 12 soal dan 360 detik.');
    }
    ...
    if ($questions->count() !== 13) {
        throw new RuntimeException('Master RA aktif harus memuat tepat tiga belas record.');
    }
    ...
    foreach ($questions as $question) {
        ...
        if (! is_array($canonical)
            ...
            throw new RuntimeException("Identitas master RA {$key} berbeda dari canonical.");
        }
        ...
        if ($activeOptions !== 0) {
            throw new RuntimeException("Master numeric RA {$key} tidak boleh mempunyai opsi aktif.");
        }
...
if ($apply) {
    ...
    if (array_sum($postflight) !== 0) {
        throw new RuntimeException('Postflight RA masih menemukan drift setelah sinkronisasi.');
    }
} else {
